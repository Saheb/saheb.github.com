...
And after the training run, you evaluate against test dataset to ensure it’s performance has improved and hasn’t degraded.

Model is the main box, the box where magic really happens, also sometimes referred to as the black box. 

Because you don’t really know what’s happening. You can peak and understand bits of it, but not really all of it.
...
Last year, I had spent some time writing a tiny transformer from scratch, to understand how modern LLMs really work. It’s from the Attention is All you Need paper that began the LLM revolution, as transformer architecture allows you to scale, which wasn’t possible in the models that came before it. If you plan to dig into transformer, don’t directly jump to it. Instead go through the journey that made it possible, the neural networks that came before it (AlexNet, ConvNet, RNNs, LSTM); that way you will be able to understand it better and also get a a wholistic understanding of the journey. Usually a limitation in one model/architecture is fixed in the future models. So it came into being by trying to remove the sequential nature of RNNs, that prevented it to scale.

Transformer is a neural network, input will be a the features I spoke about and output will the action heads, i.e. fire, ship, and target heads.
...
The boxes are in place, you run the model through the training data and it will tune its weighs, model will be trained, and ready to play. No if else loops, planing, etc. That’s the beauty and magic of it. We lose visibility and reasoning ability that we are so used to in the deterministic world. Let’s dive in.
...
Here’s are some mistakes worth noting:

I didn’t play the game enough and wasn’t giving replays the attention it deserved. That surfaced edges cases, and fundamental flaws in strategy and features. I should have done that sooner and regularly, instead of once in a while. 
...
If you can’t figure it out yourself, then it’s nor fair to expect agent to learn it
...
There were some ideas that fascinated me big time, but I couldn’t explore it fully for they required starting again from scratch. I was tempted to start over many time for there was no progress. But even then it was critical to make this decision using replay data that was available.
...
This joint head felt like the right abstraction, as all heads and decisions were joint. Fire N  ships to this planet, fire Total minu N to another planet; or some of it to reinforce, etc.
